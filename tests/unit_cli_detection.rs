...
mod tests {
    ...
    fn test_claude_code_cli_detection_with_version() {
        ...
        headers.insert(header::USER_AGENT, "claude-code/1.0.0".parse().unwrap());
    ...
    fn test_claude_desktop_cli_detection() {
        let mut headers = HeaderMap::new();
        headers.insert(header::USER_AGENT, "ClaudeDesktop/2.1.0".parse().unwrap());
        assert!(is_claude_code_cli_request(&headers));
    }
    ...
    fn test_claude_cli_detection_with_version() {
        let mut headers = HeaderMap::new();
        headers.insert(header::USER_AGENT, "claude-cli/1.2.3".parse().unwrap());
        assert!(is_claude_code_cli_request(&headers));
    }
